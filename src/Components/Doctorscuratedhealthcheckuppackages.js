...
const Doctorscuratedhealthcheckuppackages = () => {
...
const [type, settype] = useState();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!packageName || !image) {
      setError("Package name and image are required");
      return;
    }
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_KEY}addPackageCategory`,
        formData
  ...
  const GetAllPackageCategories =()=>{
    axios.get(`${process.env.REACT_APP_API_KEY}getAllPackageCategories`).then((res)=>{
  ...
  const Delete =()=>{
    const data ={
      ...
      axios.post(`${process.env.REACT_APP_API_KEY}deletePackage`,data).then((res)=>{
        GetAllPackageCategories();
  ...
  const Update =()=>{
    const formData = new FormData();
    formData.append("categoryId", Packageid);
    formData.append("name", updatepName);
    formData.append("image", updatepimage);
    formData.append("type",UpdatePackageName);
    
    
      axios.post(`${process.env.REACT_APP_API_KEY}updatePackageCategory`,formData).then((res)=>{
        GetAllPackageCategories();
        toast.success(res.data.message);
      }).catch((error)=>{
        toast.error(error.response.data.message || "An error occurred");
        ...
        <div className="page-header">
          <div className="row">
            <div className="col-sm-7 col-auto">
              <h3 className="page-title">Health Checkup Packages</h3>
              <ul className="breadcrumb">
                <li className="breadcrumb-item"><Link to="/home">Doctors Curated Health Checkup Packages </Link></li>
               
              </ul>
            </div>
            <div className="col-sm-5 col">
              <a href="#Add_Specialities_details" data-bs-toggle="modal" className="btn btn-primary float-end mt-2">+Add Package</a>
            </div>
          </div>
        </div>
        {/* /Page Header */}
...
return(
  ...
  <td>
    ...
    <a onClick={() => {setPackageid(data._id)
      ...
      setdummy(data?.image)
      setUpdatePackageName(data?.type)
          ...
          <div className="modal-body">
            <form onSubmit={handleSubmit}>
              ...
              <div className="col-12 col-sm-12">
                  <div className="mb-3">
                    <label className="mb-2">Package Type</label>
                ...
                onChange={(e) => settype(e.target.value)} type="text" placeholder='Please enter package name' className="form-control" >
                  ...
                  <option value="Doctors Curated">Doctors Curated</option>
...
export default Doctorscuratedhealthcheckuppackages
